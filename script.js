...
function renderChapter(chapterName){
    ...
    // ✅ Generates navigation links at the bottom of the page
...
// ✅ Computes bottom buttons and automatically omits the next button on Chapter 18
function renderChapterNavigation(chapterName) {
    ...
    // Layout configuration: "Back to Index" directly atop "Go to Chapter n"
    let html = `
        <button class="nav-btn back-index-btn" onclick="backToIndex()">
            ← Back to Index
        </button>
    `;

    // Only render next button if there is a real following row containing a non-empty chapter property
